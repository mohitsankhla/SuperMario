...
- (void) destroyAllBodyAndSprite {
    [spriteSheet_ removeAllChildrenWithCleanup:YES];
    b2Body *node = world_->GetBodyList();
    while (node) {
        b2Body *tmpBody = node;
        node = node->GetNext();
        world_->DestroyBody(tmpBody);
    }
}

- (void) callBackDrawPlayer:(id)sender {
...
- (void) createOutPipeMarioWithName:(NSString *)name atPos:(CGPoint)position {
    GameObject *mario = [self getSpriteByName:name];
    mario.position = position;
    [spriteSheet_ addChild:mario z:1];
    
    [mario runAction:[CCSequence actions:[CCMoveBy actionWithDuration:1.0f 
                                                             position:ccp(0, mario.contentSize.height/2+18)], 
                                          [CCCallFuncN actionWithTarget:self selector:@selector(callBackDrawPlayer:)], 
...
- (void) drawPlayer {
    ...
    if (delegate.isMarioDownWorld == NO) {
        float x, y, w, h;
        for (objPoint in [objects objects]) {
            ...
            if (objType && [objType compare:@"hero"] == NSOrderedSame) {
                if (delegate.marioStatus == kMarioSmall) {
                    _point = ccp(x, y+10.0);
                    _size = ccp(marios_standR.originalSize.width-2.0, marios_standR.originalSize.height);
                    player_ = [self getPlayerByName:@"marios_standr.png"];
                    player_.position = _point;
                    [spriteSheet_ addChild:player_];
                    [player_ createPhisicsBody:world_ 
        ...
        for (GameObject *obj in spriteSheet_.children) {
            if (obj.type == kGameObjectOutPipe) {
                if (delegate.marioStatus == kMarioSmall) {
                    ...
                    [self createOutPipeMarioWithName:@"mariom_standr.png" atPos:ccp(obj.position.x, obj.position.y)];
                }
...
- (void) drawCollideObject {
    ...
    for (objPoint in [objects objects]) {
        ...
        if (objType && [objType compare:@"ground"] == NSOrderedSame) {
        ...
        else if (objType && [objType compare:@"inUpPipe"] == NSOrderedSame) {
            PipeAndRock *inUpPipe = [self getFixedObjByName:@"pipeUp.png"];
            inUpPipe.position = ccp(_point.x, _point.y+(_size.y/2-inUpPipe.contentSize.height/2));
            inUpPipe.type = kGameObjectInUpPipe;
            inUpPipe.size = _size;
            [spriteSheet_ addChild:inUpPipe z:2];
            [inUpPipe createPhisicsBody:world_ 
                                postion:_point 
                                   size:_size 
                                dynamic:NO 
                               friction:1 
        ...
        else if (objType && [objType compare:@"outUpPipe"] == NSOrderedSame) {
            PipeAndRock *outUpPipe = [self getFixedObjByName:@"pipeUp.png"];
            outUpPipe.position = ccp(_point.x, _point.y+(_size.y/2-outUpPipe.contentSize.height/2));
            outUpPipe.type = kGameObjectOutPipe;
            outUpPipe.size = _size;
            [spriteSheet_ addChild:outUpPipe z:2];
            [outUpPipe createPhisicsBody:world_ 
                                 postion:_point 
                                    size:_size 
                                 dynamic:NO 
                                friction:1 
        ...
        else if (objType && [objType compare:@"inLeftPipe"] == NSOrderedSame) {
            PipeAndRock *inLeftPipe = [self getFixedObjByName:@"pipeLeft.png"];
            inLeftPipe.position = ccp(_point.x, _point.y+(_size.y/2-inLeftPipe.contentSize.height/2));
            inLeftPipe.type = kGameObjectInLeftPipe;
            inLeftPipe.size = _size;
            [spriteSheet_ addChild:inLeftPipe z:2];
            [inLeftPipe createPhisicsBody:world_ 
                                  postion:_point 
                                     size:_size 
                                  dynamic:NO 
                                 friction:1 
            ...
            spt.position = _point;
            spt.type = kGameObjectCoin;
            [spriteSheet_ addChild:spt];
            [coinArray_ addObject:spt];
            [spt runAction:[[coinFlash_ copy] autorelease]];
        }
...
- (void) updateCollision {
    ...
    for (pos = contactListener_->contacts_.begin(); 
        ...
        //mario是否进入水管
        if (IS_PLAYER(obj1, obj2) && (IS_INUP_PIPE(obj1, obj2) || IS_INLEFT_PIPE(obj1, obj2))) {
            if (obj1.type == kGameObjectInUpPipe && 
            ...
            if (obj2.type == kGameObjectInUpPipe && 
                player_.stkHead == kStickHeadingDown && 
        ...
        if (player_.isInvincible) {
            ...
            if (player_.isFall) {
                if (player_.isMarioMovingRight) {
                    switch (arc4random()%3) {
                        case 0:
                            [self playerRunAction:flashMarioS_WalkR1 second:flashMarioL_WalkR1 third:flashMarioL_WalkR1 tag:MARIO_RUNACTION_TAG];
                            break;
                        case 1:
                            [self playerRunAction:flashMarioS_WalkR2 second:flashMarioL_WalkR2 third:flashMarioL_WalkR2 tag:MARIO_RUNACTION_TAG];
                            break;
                        case 2:
                            [self playerRunAction:flashMarioS_WalkR3 second:flashMarioL_WalkR3 third:flashMarioL_WalkR3 tag:MARIO_RUNACTION_TAG];
                            break;
                        default:
                            break;
                    }
                    ...
                    switch (arc4random()%3) {
                        case 0:
                            [self playerRunAction:flashMarioS_WalkL1 second:flashMarioL_WalkL1 third:flashMarioL_WalkL1 tag:MARIO_RUNACTION_TAG];
                            break;
                        case 1:
                            [self playerRunAction:flashMarioS_WalkL2 second:flashMarioL_WalkL2 third:flashMarioL_WalkL2 tag:MARIO_RUNACTION_TAG];
                            break;
                        case 2:
                            [self playerRunAction:flashMarioS_WalkL3 second:flashMarioL_WalkL3 third:flashMarioL_WalkL3 tag:MARIO_RUNACTION_TAG];
                            break;
                        default:
                            break;
                    }
...
- (void) stopCheckOut {
    [self unschedule:@selector(updateCollision)];
    [self unschedule:@selector(checkoutJump:)];
    [self unschedule:@selector(updatePlayerPositon:)];
    [self unschedule:@selector(updatePlayerFire:)];
    [self unschedule:@selector(updateOtherObjPosition)];
